...
前置工作完成，执行实例化方法：beanFactory.preInstantiateSingletons()：

同样在实例化的前面有些前置操作，不做详细介绍。讲几个点儿：

1. RootBeanDefinition: Spring BeanFactory 工厂中 bean 属性合并的定义类; 使用阿里打印 JSON 串的时候，会报错；因为这个类中包含的 bean 还没有实例化，无法打印；
2. bean 如果实现了 FactoryBean 的接口，最终会通过 getObject() 这个接口获取对象; 后续详细介绍；
3. 没有实现 FactoryBean 接口，则通过 getBean(Name) 进行 bean 的实例化;

bean 实例化真正干活的是 doGetBean 这个方法，在这个方法中完成了 bean 的实例化。看下是否是通过反射实现 bean 的实例化：

还是捡几个点儿说下：
1. 先判断从缓存中获取 bean 对象, 获取到了返回 bean 对象，继续后续操作; Object sharedInstance = getSingleton(beanName);
2. 缓存中没有 bean 对象，则进行实例化创建；在这个方法之前还有一堆逻辑，但是不重要；直接看 createBean 方法；

在 createBean 同样是一大堆逻辑，简单介绍下这几个：
1. Object bean = resolveBeforeInstantiation(beanName, mbdToUse); BeanPostprocessor 处理执行是否返回了一个 bean；

然后忽略逻辑，找到执行 return createBean(beanName, mbd, args); 创建 bean 实例；同样是一堆儿的逻辑，
直接找到 instanceWrapper = createBeanInstance(beanName, mbd, args); 方法，然后忽略一大堆的逻辑，找到 return instantiateBean(beanName, mbd);
再忽略一大堆逻辑，
