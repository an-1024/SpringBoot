...
1. setConfigLocations(configLocations): 设置配置文件路径，并准备 Java 的运行环境：customizePropertySources;
2. prepareRefresh(): 准备 spring IOC 运行环境
3. obtainFreshBeanFactory创建 bean 工厂: DefaultListableBeanFactory, prepareBeanFactory设置工厂属性
   1. 类图如下：
   ![](../photos/2.DefaultListableBeanFactory.png)
4. 将 bean 的增强处理器放入 bean 工厂中，执行 bean 的增强处理器 postProcessBeanFactory(beanFactory)
5. 实例化的前期工作：
