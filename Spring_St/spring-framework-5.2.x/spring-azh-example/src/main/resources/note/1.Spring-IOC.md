...
```
前面提到过执行配置类注解的解析是通过执行后置处理器的时候完成的。再添加了 component-scan 标签后，Spring 会获取到一个名叫
org.springframework.context.annotation.internalConfigurationAnnotationProcessor；这是 Spring 内部管理的配置注释处理器的 bean 名称；
通过 beanFactory.getBean(ppName, BeanDefinitionRegistryPostProcessor.class) 创建了  ConfigurationAnnotationProcessor 类，并执行
