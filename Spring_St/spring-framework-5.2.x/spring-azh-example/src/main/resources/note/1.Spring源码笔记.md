...
# 配置类注解的识别-ConfigurationClassPostProcessor
...
![](../photos/6.BeanDefinition结构.drawio.png)
...
public class ConfigurationClassPostProcessor implements BeanDefinitionRegistryPostProcessor,
   ...
   public void postProcessBeanDefinitionRegistry(BeanDefinitionRegistry registry) {
      ...
      // 处理配置类的 bean 定义信息以及内部逻辑
      processConfigBeanDefinitions(registry);
   }
}
```
ConfigurationClassPostProcessor 类实现了接口 BeanDefinitionRegistryPostProcessor，而 BeanDefinitionRegistryPostProcessor 接口
继承了 BeanFactoryPostProcessor，所以在执行 BeanFactoryPostProcessor 后置处理器的时候，就会执行 ConfigurationClassPostProcessor 的
的方法处理配置类的注解，完成相应的逻辑处理。

所以我们在日常开发配置中，比如数据库初始化赋值等，就是在这个过程完成的。

##  processConfigBeanDefinitions(registry)
总结一下这个方法干了什么：
1. 获取所有的 BeanDefinition 对象，并筛选出具有注解的 BeanDefinition 对象；
   1. org.springframework.context.annotation.ConfigurationClassUtils.checkConfigurationClassCandidate 判断 BeanDefinition 
   的 attribute 是否包含 configurationClass；如果是，输出该 Bean 已经被处理过；反之针对注解类型进行分类处理：
   AnnotatedBeanDefinition、 AbstractBeanDefinition、其他类型，最终的目的是获取 AnnotationMetadata 元数据信息；根据元数据的属性判断
   这个类是否属于 configurationClass，是则返回 true，将这个类填入 BeanDefinitionHolder 的集合 configCandidates 中。
2. 获取到所有注解的 beanDefinition 对象之后，接着需要对注解中的参数值解析处理：在实际的注解应用中，我们常常会在注解中填写参数，来完成
一些必要的功能。比如 @RestController 指定路径， @Service("xxxx") 指定服务名称等。
   1. 判断注解的 beanDefinition 对象是否包含自定义的命名生成器：如图：
   ![img.png](../photos/7.BeanDefinition名称.png)
   可以看到 beanDefinition 的名称有包含包名的，有名称后面带有编号的，也有不包含包的。如果我们不自定义命名生成器，那 Spring 会按照自己定义
   的规则来生成。org.springframework.beans.factory.support.BeanNameGenerator 这是实现命名规则的接口
   2. 获取注解解析器，根据不同的解析器调用 parse 解析。核心处理方法：org.springframework.context.annotation.ConfigurationClassParser.doProcessConfigurationClass
      1. 递归处理内部配置类；
      2. 处理 @PropertySource注解逻辑：解析 properties 文件，读取配置文件参数(.yml、.properties)；
      3. 处理 @ComponentScan 和 @ComponentScans 注解逻辑：扫描指定包路径下的类以及指定类的包路径下的类；
      4. 处理 @Import 注解逻辑：通过快速导入的方式实现把实例加入spring的IOC容器中；
      5. 处理 @ImportSource 注解逻辑：处理 @ImportSource 注解引入的配置文件(.xml)；
      6. 处理加在方法上的 @Bean 注解 ： 被 @Bean标注的方法会生成一个由 Spring 容器管理的 bean；
      7. 处理接口方法上的 @Bean 注解：从 jdk8 开始，接口可以有自己的实现；
      8. 解析配置类的父类，如果配置类继承了某个父类，那么配置类的父类也会被解析
3. 剩余的代码没有涉及到注解的了，所以不看了。
...
<beans xmlns="http://www.springframework.org/schema/beans"
	   ...
	   xmlns:context="http://www.springframework.org/schema/context"
	   xsi:schemaLocation="http://www.springframework.org/schema/beans
	   https://www.springframework.org/schema/beans/spring-beans-3.0.xsd http://www.springframework.org/schema/context https://www.springframework.org/schema/context/spring-context.xsd">

	<!--指定扫描包的路径-->
	<context:component-scan base-package="com.azh.configannotation.config"/>
</beans>
```
前面提到过执行配置类注解的解析是通过执行后置处理器的时候完成的。在添加了 component-scan 标签后，Spring 会获取到一个名叫
org.springframework.context.annotation.internalConfigurationAnnotationProcessor。 识别到这个标签后，Spring 会创创建一个 
RootBeanDefinition，该类包含将要实例化的 bean-》ConfigurationClassPostProcessor
```java
...
```
最后通过 `beanFactory.getBean(ppName, BeanDefinitionRegistryPostProcessor.class)` 创建了  ConfigurationAnnotationProcessor 类，并执行
该类的 postProcessBeanDefinitionRegistry(BeanDefinitionRegistry registry) 方法完成注解解析。
具体细节不赘述了。可以自行 Debug 一下。看下过程。

### @Import 注解自动注入分析
前面提到过 Spring 在获取到解析器之后会调用不同的解析进行注解解析，这里以注解为列：在 org.springframework.context.annotation.ConfigurationClassParser.parse(java.util.Set<org.springframework.beans.factory.config.BeanDefinitionHolder>)
这个方法中主要对注解类进行区分：
1. BeanDefinition 类型是否是 AnnotatedBeanDefinition 类型：AnnotatedBeanDefinition 是 BeanDefinition 子接口之一，该接口扩展了
BeanDefinition 的功能，其用来操作注解元数据。一般情况下，通过注解方式得到的 Bean（@Component、@Bean），其 BeanDefinition 类型都是该接口的实现类； 
2. BeanDefinition 类型是否是 AbstractBeanDefinition 类型：AbstractBeanDefinition 是 BeanDefinition 的子抽象类，也是其他 
BeanDefinition 类型的基类，其实现了接口中定义的一系列操作方法，并定义了一系列的常量属性，这些常量会直接影响到 Spring 实例化 Bean 时的策略
3. 如果 BeanDefinition 类型不是上面两种类型, 另行处理。

根据 BeanDefinition 的类型调用不同 parse 方法开始进行解析。核心处理方法：org.springframework.context.annotation.ConfigurationClassParser.doProcessConfigurationClass；
在这个方法中，调用 org.springframework.context.annotation.ConfigurationClassParser.processImports 完成注解 @Import 的解析。
```java
...
```
根据上述源代码分析，这个方法主要根据 @Import 注解类的类型分别实现对应的逻辑处理：
1. BeanDefinition 是否是 ImportSelector.class 类型，如果是，接着判断该类型是否是 DeferredImportSelector 类型，如果是，会将这个 
BeanDefinition 放入 deferredImportSelectors 集合中，然后在所有注解处理完毕之后，才会处理这个集合中的 BeanDefinition，调用
org.springframework.context.annotation.ConfigurationClassParser.DeferredImportSelectorHandler.process 这个方法完成逻辑处理
(**SpringBoot 中的类的自动加载就是 通过实现 org.springframework.context.annotation.DeferredImportSelector.Group.process 这个接口的
的方法完成的**)；如果不是 DeferredImportSelector 类型，则直接开始处理 @Import 注解逻辑；
2. BeanDefinition 是否是 ImportBeanDefinitionRegistrar 类型，如果是表明该注解类有依赖，需要线完成其他 bean 的注册；
3. 如果不属于上述两种类型，则递归调用 org.springframework.context.annotation.ConfigurationClassParser.processConfigurationClass
完成解析；

## 小结@Import注解
经过上面对注解的分析，我们可以总结一 SpringBoot 的自动加载类的过程：
1. Spring 正对于注解的解析是从执行后置处理器 BeanFactory 开始的，Spring 中获取内置类 ConfigurationAnnotationProcessor，执行这个类
的后置处理方法 postProcessBeanDefinitionRegistry 完成注解解析；
2. 其中在解析的过程中针对于 @Import 注解是通过调用 processImports 方法完成注解的解析；
3. 在 processImports 方法中通过 geImports 获取到实现 DeferredImportSelector 接口的 AutoConfigurationImportSelector 类；通过该类的
process 获取到 AutoConfigurationEntry，然后调用 org.springframework.boot.autoconfigure.AutoConfigurationImportSelector#getCandidateConfigurations
获取到 SpringBoot META-INF/spring.factories 文件中的配置信息，完成 @Import 注解的自动注入。

# registerBeanPostProcessors(beanFactory)：实例化注册所有 bean
org.springframework.context.support.PostProcessorRegistrationDelegate.registerBeanPostProcessors(org.springframework.beans.factory.config.ConfigurableListableBeanFactory, org.springframework.context.support.AbstractApplicationContext)
这个方法处理完成了 Bean 的注册与实例化。
1. 找到所有实现了 BeanPostProcessor 接口的类；
2. BeanPostProcessor 的目标计数，加 1；
3. 添加 BeanPostProcessorChecker (主要用于记录信息) 到 beanFactory 中
   1. 该类后置处理器前置方法：postProcessBeforeInitialization 直接返回 bean；
   2. 该类后置处理器后置方法：postProcessAfterInitialization 对 bean 的角色进行记录；
4. 定义存放实现了不同接口的集合，以及存放实现对应接口 bean 名称的集合：priorityOrderedPostProcessors、internalPostProcessors、
orderedPostProcessorNames、nonOrderedPostProcessorNames；
5. 遍历循环实现 BeanPostProcessor 接口的 Bean，然后判断 Bean 的类型，将Bean 放入不同的集合中；以下是对实现 BeanPostProcessor 接口的
说明：
   1. BeanPostProcessor：bean 初始化前后的时候会调用该接口的方法：postProcessBeforeInitialization、postProcessAfterInitialization；
   2. DestructionAwareBeanPostProcessor：bean 销毁的时候会调用该接口的方法：postProcessBeforeDestruction；
   3. MergedBeanDefinitionPostProcessor：合并 bean 的属性信息：postProcessMergedBeanDefinition；
   4. InstantiationAwareBeanPostProcessor： bean 实例化前后需要做的事情：postProcessBeforeInstantiation、postProcessAfterInstantiation；
   5. SmartInstantiationAwareBeanPostProcessor：bean 的类型推断、构造器推断、bean 实例化循环依赖的解决：predictBeanType、determineCandidateConstructors、
      getEarlyBeanReference；
6. 如果该 bean 的类型属于 PriorityOrdered(其他类型的 bean 的操作流程与这个相同)：
   1. 获取该 bean 的实例；
   2. 将该实例添加到 priorityOrderedPostProcessors 集合中；
   3. 如果 bean 实现 BeanPostProcessor 接口的实例实现了 MergedBeanDefinitionPostProcessor 接口，
   4. 则将 bean 添加到 internalPostProcessors 集合中；
   5. 对实现了 PriorityOrdered 接口 bean 实例排序；
   6. 将实现了 PriorityOrdered 接口的 bean 实例添加到 beanFactory 中；
7. 重新注册用于将内部 bean 检测为 ApplicationListeners 的后处理器；

这里我们可以尝试修改一下 Spring 的源码。就是在遍历循环 BeanPostProcessor 的时候，只处理了 PriorityOrdered 接口 bean 实例，
另外两个接口的 bean 实例只是将对应的 bean 名称添加到了对应接口类的名称中：orderedPostProcessorNames、nonOrderedPostProcessorNames；
可以尝试直接在这个循环里处理对应的 bean 实例，而不是在后面再次循环 这两个名称集合，然后再处理。

# 国际化处理
这块不研究了：initMessageSource();
# 初始化监听器
初始化广播器：监听事：initApplicationEventMulticaster();关于这个知道观察者模式就行。在 SpringBoot 中就有很多监听器的使用。
在 Spring 中，观察者模式具体实现如下：

1. 事件：被观察者具体要执行的动作；
2. 监听器：观察者，可能存在多个，接收不同的事件来做不同的处理工作；
3. 多播器：把被观察者遍历观察者通知消息的操作拿出来委托给一个多播器来进行消息通知，或者说通过观察者进行不同的操作；
4. 事件源：谁来调用或者执行发布具体的事件；

Spring 中观察者模式的执行流程：

1. 事件源来发布不同的事件；
2. 当发布事件之后会调用多播器的方法来进行事件的广播操作，由多播器触发具体的监听器去执行操作；
3. 监听器接收到具体的事件之后，可以验证匹配是否能处理当前事件。如果可以，直接处理。如果不行，则不做任何操作；
